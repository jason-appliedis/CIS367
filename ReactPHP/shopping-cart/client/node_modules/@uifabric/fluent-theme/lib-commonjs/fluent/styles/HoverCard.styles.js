"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
var tslib_1 = require("tslib");
var commonCardStyles = function (props) {
    var theme = props.theme;
    var effects = theme.effects;
    return {
        border: 'none',
        boxShadow: effects.elevation16,
        borderRadius: effects.roundedCorner2,
        selectors: {
            '.ms-Callout-main': { borderRadius: effects.roundedCorner2 },
        },
    };
};
exports.ExpandingCardStyles = function (props) {
    return {
        root: tslib_1.__assign(tslib_1.__assign({}, commonCardStyles(props)), { width: 320 }),
        expandedCard: {
            selectors: {
                ':before': {
                    width: 272,
                },
            },
        },
    };
};
exports.PlainCardStyles = function (props) {
    return {
        root: tslib_1.__assign({}, commonCardStyles(props)),
    };
};
//# sourceMappingURL=HoverCard.styles.js.map